...
io.on("connection", (socket: Socket) => {
    ...
    socket.on('createLobby', ({ playerName, lobbyCode, isSpectator }) => {
        ...
        if (lobbies[lobbyCode]) {
            socket.emit("createLobbyResponse", {
                success: false,
                message:
                    "Lobby code already exists. Please choose a different code.",
                lobby: null,
            });
            return;
        }
    ...
    socket.on("playerShoot", ({ targetPlayerId, lobbyCode, damage = 10 }) => {
        const lobby = lobbies[lobbyCode];
        if (!lobby) return;
    ...
    socket.on("startGame", ({ lobbyCode }) => {
        ...
        console.log(
            `Game started for lobby ${lobbyCode}. Notifying ${lobby.players.length} players.`,
        );
    ...
    socket.on("playerColorDetected", ({ lobbyCode, username, rgb }) => {
        console.log(
            `Player ${username} in lobby ${lobbyCode} detected color: RGB(${rgb.r}, ${rgb.g}, ${rgb.b})`,
        );
        ...
        console.log(
            `Stored RGB(${rgb.r}, ${rgb.g}, ${rgb.b}) for player ${username}`,
        );
        ...
        lobby.players.forEach((p, index) => {
            const colorInfo =
                p.r !== undefined && p.g !== undefined && p.b !== undefined
                    ? `RGB(${p.r}, ${p.g}, ${p.b})`
                    : "No color detected";
            console.log(
                `${index + 1}. ${p.name} (${p.isHost ? "Host" : "Player"}) - ${colorInfo}`,
            );
        });
        console.log(`===============================================\n`);
